...
class conv_layer(Layer):
    def __init__(self,input_shape,kernel_size,depth):
        #unpacking input
        input_depth, input_height,imput_width = input_shape # ex touple cifa (3,32,32) or mnist (1,28,28)
        ...
        self.kernels = np.random.randn(self.kernel_shape) # random init of kernels
        self.biases = np.random.randn(self.output_shape) #random init of biases

    def forward(self,image):
        self.image = image
        self.output = np.copy(self.biases) #each output equals bias + computed thing
        ...
        for i in range(self.depth): #output depth
            for j in range(self.input_depth): #input depth
                self.output += singal.correlate2d(self.input[j],
                                                self.kernelsp[i,j],
                                                'valid')
        return self.output
    ...
    def backward(self, output_grad , lr):
        ...
        """
        kernel_grad = np.zeros(self.kernel_shape) # initialize empty kernel placeholder K
        ...
        for i in range(self.depth):
            for j in range(self.input_depth):
                #gradient of 
                kernel_grad[i,j] = signal.correlate2d(self.image[j],output_grad[i],'valid')
